...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Asus\\Documents\\pretest_fmri\\guess\\guess_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    instructions3_text = visual.TextStim(win=win, name='instructions3_text',
        text='Raten Sie nicht laut, sondern versuchen Sie erstmal möglichst viele Assoziationen zum linken Wort leise in Ihrem Kopf zu generieren.\n\nZum Beispiel, für das Wort "Sommerferien" können Sie vielleicht die folgenden Assoziationen haben: Urlaub, Strand, Reisen, Ferienjob, Eis, und so weiter. Sie haben 5 Sekunden Zeit nach Erscheinen des linken Wortes, um mindestens einen stillen Rateversuch zu machen. Danach müssen Sie angeben, ob Sie einen oder mehrere Rateversuche generieren konnten, indem Sie die Tasten F, G oder H drücken.\n\nF) Keinen Rateversuch generiert G) Einen Rateversuch generiert H) Mehr als einen Rateversuch generiert\n\nDrücken Sie die LEERTASTE, um fortzufahren.',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    iti_learning_blank = visual.TextStim(win=win, name='iti_learning_blank',
        text=None,
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
    ...
    recall_instructions_text = visual.TextStim(win=win, name='recall_instructions_text',
        text='Im nächsten Teil des Experiments müssen Sie das Wortpaar zu jedem auf dem Bildschirm angezeigten Wort abrufen.\n\nVersuchen Sie, das Wort (5 Sekunden) abzurufen, und geben Sie dann an, ob Sie sich an das Wort erinnern, indem Sie die Tasten F (Kann mich nicht erinnern) und G (Kann mich erinnern) verwenden.\n\nWenn Sie sich an das Wort erinnern können, fragen wir Sie nach dem letzten Buchstaben des Wortes. Sie können aus 4 Optionen wählen (Tasten 1-4).\n\nBevor Sie starten, üben Sie die Aufgabe.\n\nDrücken Sie die LEERTASTE, um mit der Übung zu beginnen.',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    guess_conditions = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('guess_trial_randomization.xlsx'), 
        seed=None, 
    )
    ...
    for thisGuess_condition in guess_conditions:
        ...
        while continueRoutine and routineTimer.getTime() < 0.1:
            ...
            if welcome_text.status == STARTED:
                ...
                if tThisFlipGlobal > welcome_text.tStartRefresh + 0.1-frameTolerance:
    ...
    for thisPractice_trial in practice_trials:
        ...
        while continueRoutine and routineTimer.getTime() < 1.0:
            ...
            if iti_practice_blank.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > iti_practice_blank.tStartRefresh + 1.0-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        elif guess_reached.keys == "g":
            feedback = feedback + "\nEinen Rateversuch gemacht."
        elif guess_reached.keys == "h":
            feedback = feedback + "\nMehrere Rateversuche gemach"
        else:
    ...
    for thisLearning_trial in learning_trials:
        ...
        while continueRoutine and routineTimer.getTime() < 1.0:
            ...
            # if iti_learning_blank is starting this frame...
            if iti_learning_blank.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                iti_learning_blank.frameNStart = frameN  # exact frame index
                iti_learning_blank.tStart = t  # local t and not account for scr refresh
                iti_learning_blank.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(iti_learning_blank, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'iti_learning_blank.started')
                # update status
                iti_learning_blank.status = STARTED
                iti_learning_blank.setAutoDraw(True)
            
            # if iti_learning_blank is active this frame...
            if iti_learning_blank.status == STARTED:
                # update params
                pass
            
            # if iti_learning_blank is stopping this frame...
            if iti_learning_blank.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > iti_learning_blank.tStartRefresh + 1.0-frameTolerance:
                    # keep track of stop time/frame for later
                    iti_learning_blank.tStop = t  # not accounting for scr refresh
                    iti_learning_blank.tStopRefresh = tThisFlipGlobal  # on global time
                    iti_learning_blank.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'iti_learning_blank.stopped')
                    # update status
                    iti_learning_blank.status = FINISHED
    ...
    for thisTest_practice_trial in test_practice_trials:
        ...
        while continueRoutine and routineTimer.getTime() < 1.0:
            ...
            if iti_blank.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > iti_blank.tStartRefresh + 1.0-frameTolerance:
                    # keep track of stop time/frame for later
    ...
    for thisTest_trial in test_trials:
        ...
        task_break.skipped = continueRoutine and not (task_trial < 2)
    ...
    for thisSemantic_mapping in semantic_mapping:
        ...
        item = data.Routine(
            name='item',
            components=[semantic_map_item, living_nonliving, end_mapping, living_nonliving_text],
        )
        ...
        if list_item in target_types:
            thisExp.addData('sm_trial_type', target_types[list_item])
        else:
            thisExp.addData('sm_trial_type', "Mediator")
        ...
        while continueRoutine and routineTimer.getTime() < 5.0:
            ...
            if living_nonliving_text.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > living_nonliving_text.tStartRefresh + 5.0-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        print("mapping_trial", mapping_trial)
        ...
        mapping_break.skipped = continueRoutine and not (mapping_trial < 2)
